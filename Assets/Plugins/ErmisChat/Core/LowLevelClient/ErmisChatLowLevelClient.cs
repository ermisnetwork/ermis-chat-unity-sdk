...
#if ERMIS_TESTS_ENABLED
[assembly: InternalsVisibleTo("ErmisChat.Tests")] //ErmisTodo: verify which Unity version introduced this
#endif

namespace Ermis.Core.LowLevelClient
{
    /// <inheritdoc cref="IErmisChatLowLevelClient"/>
    public class ErmisChatLowLevelClient : IErmisChatLowLevelClient
    {
        public const string MenuPrefix = "Ermis/";

        public static readonly Uri ServerBaseUrl = new Uri("wss://api-dev.ermis.network");
        ...
        [Obsolete(
            ...
            "Please use the OwnUser object returned from ErmisChatClient.Connected event. This property will  be removed in the future.")]
        ...
        //ErmisTodo: wrap all params in a ReconnectPolicy object
        ...
        /// <summary>
        /// Use this method to create the main client instance or use ErmisChatClient constructor to create a client instance with custom dependencies
        /// </summary>
        /// <param name="authCredentials">Authorization data with ApiKey, UserToken and UserId</param>
        public static IErmisChatLowLevelClient CreateDefaultClient(AuthCredentials authCredentials,
            IErmisClientConfig config = default)
        {
            if (config == null)
            {
            ...
            var websocketClient
                = ErmisDependenciesFactory.CreateWebsocketClient(logs, isDebugMode: config.LogLevel.IsDebugEnabled());
            ...
            return new ErmisChatLowLevelClient(authCredentials, websocketClient, httpClient, serializer,
        ...
        /// <summary>
        /// Strip invalid characters from a given Ermis user id. The only allowed characters are: a-z, 0-9, @, _ and -
        /// </summary>
        ...
        public ErmisChatLowLevelClient(AuthCredentials authCredentials, IWebsocketClient websocketClient,
            ...
            IApplicationInfo applicationInfo, ILogs logs, IErmisClientConfig config)
        {
            ...
            _logs.Prefix = "[Ermis Chat] ";

            _requestUriFactory = new RequestUriFactory(authProvider: this, connectionProvider: this, _serializer);

            _httpClient.AddDefaultCustomHeader("stream-auth-type", DefaultErmisAuthType);
            var header = BuildErmisClientHeader(applicationInfo);
        ...
        {
            return _authCredentials.UserToken;
        }
        ...
        public void Connect()
        {
            ...
            //ErmisTodo: hidden dependency on SetUser being called
        ...
        public void SeAuthorizationCredentials(AuthCredentials authCredentials)
        {
            ...
            {
                throw new ErmisMissingAuthCredentialsException(
        ...
        {
            TryCancelWaitingForUserConnection();
            //ErmisTodo: remove this, this cannot be used when internal disconnect due to expired token. Perhaps we should allow user to Suspend() and Unsupend() the client reconnection
        ...
        //ErmisTodo: move this to injected config object
        ...
        public async Task FetchAndProcessEventsSinceLastReceivedEvent(IEnumerable<string> channelCids)
        {
            ...
            //ErmisTodo: according to Android SDK there's an error if there are > 1000 events 
            ...
            {
                // ErmisTodo: check if we can not serialized this again. Investigate adding a custom EventsJsonConverter that would populate the list as serialized strings
        ...
        string IAuthProvider.ErmisAuthType => DefaultErmisAuthType;
            ...
            ITokenProvider tokenProvider, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                //ErmisTodo: pass the cancellation token here cancellationToken
        ...
        private const string DefaultErmisAuthType = "jwt";
        ...
        private readonly IErmisClientConfig _config;
        ...
        {
            //ErmisTodo: resolve issue that expired token also triggers connection confirmed that gets immediately disconnected
...
#pragma warning restore 0618
            ...
            where TEvent : ILoadableFrom<TDto, TEvent>, new()
        {
            ...
            {
                throw new ErmisDeserializationException(content, typeof(TDto), e);
            }
        ...
        //ErmisTodo: make it more clear that we either receive full set of credentials or apiKey, userId and the token provider
        ...
        private void LogErrorIfUpdateIsNotBeingCalled()
        {
            ...
            //ErmisTodo: temporarily disable update monitor when tests are enabled -> investigate why some tests trigger this error
#if !ERMIS_TESTS_ENABLED
            const int timeout = 2;
            Task.Delay(timeout * 1000, _updateMonitorCts.Token).ContinueWith(t =>
            {
                ...
                {
                    _logs.Error(
                        $"Connection is not being updated. Please call the `{nameof(ErmisChatLowLevelClient)}.{nameof(Update)}` method per frame. Connection state: {ConnectionState}");
                }
            }, _updateMonitorCts.Token);
#endif
        }

        private static string BuildErmisClientHeader(IApplicationInfo applicationInfo)
        {
            var sb = new StringBuilder();
            sb.Append($"ermis-chat-unity-client-");
        ...
        {
            return _authCredentials.UserId;
        }
    }
}
