...
namespace ErmisChat.Tests.LowLevelClient.Integration
{
    /// <summary>
    ...
    internal abstract class BaseIntegrationTests
    {
        ...
        protected static IErmisChatLowLevelClient LowLevelClient => ErmisTestClients.Instance.LowLevelClient;

        /// <summary>
        /// Id of other user than currently logged one
        /// </summary>
        protected static string SecondUserId => ErmisTestClients.Instance.AdminSecondaryCredentials.UserId;

        protected static OwnUser LowLevelClientOwnUser => ErmisTestClients.Instance.LowLevelClientOwnUser;

        protected static IEnumerator ReconnectClient() => ErmisTestClients.Instance.ReconnectLowLevelClientClient();
            ...
            int initTimeoutMs = 150)
        {
            ...
            while (attempt <= maxAttempts)
            {
                ...
                catch (ErmisApiException ermisApiException)
                {
                    // Check for "Too many requests" error
                    if (ermisApiException.IsRateLimitExceededError())
                    {
                    ...
                    if (ermisApiException.Code == 504)
                    {
                        continue;
                    }
                    
                    Debug.LogException(ermisApiException);
                }
        ...
        private async Task DeleteTempChannelsAsync()
        {
            ...
            {
                if (ermisApiException.Code == ErmisApiException.RateLimitErrorHttpStatusCode)
                {
