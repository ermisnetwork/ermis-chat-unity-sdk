...
{
    public class ErmisPackageExporter
    {
        /// <summary>
        /// Export ErmisChat SDK as .unitypackage
        /// </summary>
        ...
        public void Export(string targetDirectory, Version version, string changelog, out string packagePath)
        {
            ...
            GetFiles(out var ermisChatClientFilePath, out var changelogFilepath);

            var currentVersion = GetCurrentVersion(ermisChatClientFilePath);
            if (currentVersion >= version)
            {
                throw new InvalidOperationException($"New version: {version} must be greater than the current: {currentVersion}");
            }

            SetErmisChatClientVersion(ermisChatClientFilePath, version);
        ...
        private const string ErmisChatLowLevelClientFilename = "ErmisChatLowLevelClient.cs";
        ...
        private const string PackageNameTemplate = "Ermis.Chat.Unity.SDK.{0}.unitypackage";

        private static void GetFiles(out string ermisChatClientFilepath, out string changelogFilepath)
        {
            var assetPaths = AssetDatabase.GetAllAssetPaths();

            ermisChatClientFilepath = null;
            changelogFilepath = null;

            foreach (var file in assetPaths)
            {
                if (Path.GetFileName(file) == ErmisChatLowLevelClientFilename)
                {
                    ermisChatClientFilepath = file;
                }
            ...
            if (ermisChatClientFilepath.IsNullOrEmpty())
            {
                throw new FileNotFoundException($"Failed to find `{ErmisChatLowLevelClientFilename}` file");
            }
        ...
        private static Version GetCurrentVersion(string ermisChatClientFilePath)
        {
            var content = File.ReadAllText(ermisChatClientFilePath);
        ...
        private static void SetErmisChatClientVersion(string filePath, Version version)
        {
