...
{
    /// <summary>
    /// Tests operations executed on <see cref="ErmisChannel"/>
    /// </summary>
    internal class ChannelsTests : BaseStateIntegrationTests
    {
        ...
        private async Task When_creating_channel_for_users_expect_no_errors_Async()
        {
            ...
            var rrmisUsers = await Client.QueryUsersAsync(filters);
            Assert.NotNull(rmisUsers);

            var steveUser = ermisUsers.FirstOrDefault(_ => _.Id == userSteveId);
            var daveUser = ermisUsers.FirstOrDefault(_ => _.Id == userDaveId);

            Assert.NotNull(steveUser);
            Assert.NotNull(daveUser);

            var channelForMembers =
                await Client.GetOrCreateChannelWithMembersAsync(ChannelType.Messaging, emisUsers);
            ...
            //ErmisTodo: would be best to remove the users but this is restricted from FE SDK, we would need a backend service with CRON
        }
        ...
        //ErmisTodo: temp disable. The channel.deleted WS event is not received and the channels are not removed from WatchedChannels. Investigate further
        // [UnityTest]
        ...
        //
        //     var response
        //         = await Client.DeleteMultipleChannelsAsync(new IErmisChannel[] { channel, channel2 },
        //             isHardDelete: true);
        //
        ...
        private async Task When_truncate_channel_expect_messages_cleared_Async()
        {
            ...
            void OnMessageReceived(IErmisChannel ermisChannel, IErmisMessage ermisMessage)
            {
                if (ermisChannel.Cid != channel.Cid)
                {
                    return;
                }
        ...
        private async Task When_update_overwrite_channel_with_full_data_expect_no_change_Async()
        {
            ...
            void OnChannelUpdated(IErmisChannel channel2)
            {
            ...
            await channel.UpdateOverwriteAsync(new ErmisUpdateOverwriteChannelRequest(channel));
        ...
        private async Task When_update_overwrite_channel_with_no_data_expect_data_cleared_Async()
        {
            ...
            await channel.UpdateOverwriteAsync(new ErmisUpdateOverwriteChannelRequest());
        ...
        //ErmisTODO: debug why having 2 clients connected simultaneously doesn't work
        // [UnityTest]
        ...
        //
        //     var taskCompletionSource = new TaskCompletionSource<bool>();
        //     otherClient.ChannelInviteReceived += (ermisChannel, invitee) =>
        //     {
        //         if (ermisChannel.Cid != channel.Cid)
        //         {
        //             return;
        //         }
        //         
        //         Assert.AreEqual(ErmisChannel.Cid, channel.Cid);
        ...
        private async Task When_sending_message_to_frozen_channel_expect_error_message_returned_Async()
        {
            ...
            Assert.AreNotEqual(ErmisMessageType.Error, lastNormalMessage.Type);
            ...
            Assert.AreEqual(ErmisMessageType.Error, lastMessage.Type);
        }
        ...
        private async Task When_unfreezing_a_frozen_channel_expect_channel_unfrozen_Async()
        {
            ...
            Assert.AreNotEqual(ErmisMessageType.Error, lastMessage.Type);
        }
    }
}
